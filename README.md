...
The pseudo-code of the matching function is represented in the Figure 1.

```
Figure 1: Knuth Morris Pratt Matching function pseudocode
```
### 2.3 Failure Function
...
```
Figure 2: Knuth Morris Pratt failure function pseudocode
```
#### 2.3.1 Failure function as a Finite State Automata
...
To make the User comfortable in using the tool, I developed a graphical user
interface through which it is easy to select which KMP implementation run
(software or hardware).

The code flow is lead by Python, if the user calls the software version, python
calls its kmp function, otherwise it calls the host and wait for the result to be
computed.

Both software and hardware implementations return the same result.
...
```
Figure 3: Repeat Analyzer Benchmark
```
...
Without the pre-processing phase the speed up will result in 100x, the ker-
nel function on FPGA is very efficient. Some improvements can be done in
order to optimize the pre-processing.
...
11. Vitis Unified Software Platform Documentation
12. Pattern Matching Performance Comparison as Big Data Analysis Re-
comendations for Hepatitis C Virus (HCV) Sequence DNA
13. RepeatAnalyzer: a tool for analysing and managing short-sequence repeat
data
14. RepeatAnalyzer
15. DNA sequence matching processor using FPGA and JAVA interface
16. What is the best software for sequence alignment?
17. Knuth-Morris-Pratt Algorithm
18. Basic HLS Tutorial
19. KMP algorithm implementation on FPGA
20. KMP based tools - YASS
21. The BLAST Sequence Analysis Tool
22. Edlib: a C/C++ library for fast, exact sequence alignment using edit
distance
23. SMART: String Matching Algorithms Research Tool
24. Grep like tool for file and dir search


25. Clustal: Multiple sequence alignment
26. Genedoc
27. https://github.com/felipealbrecht/Genoogle
28. Full genome multiple sequence alignments
29. An Analysis on three Influential DNA Sequencing Algorithms
